--[[
    ...
    YAPIMCI: NOXYORJ
    Özellikler: Otomatik Hawk Açılış Animasyonu, Anlık Profil, %100 API Mimarisi
]]
...
function Luaware:Window(Config)
    ...
    -- =======================================
    -- LUAWARE OTOMATİK AÇILIŞ ANİMASYONU
    -- =======================================
    local IntroLogo = Instance.new("ImageLabel", sg)
    IntroLogo.Size = UDim2.new(0, 0, 0, 0) -- Başlangıçta görünmez (0x0)
    IntroLogo.Position = UDim2.new(0.5, 0, 0.5, 0)
    IntroLogo.BackgroundTransparency = 1
    IntroLogo.Image = "rbxassetid://13570069771"
    IntroLogo.ImageTransparency = 1

    local IntroTitle = Instance.new("TextLabel", sg)
    IntroTitle.Size = UDim2.new(0, 200, 0, 30)
    IntroTitle.Position = UDim2.new(0.5, -100, 0.5, 30)
    IntroTitle.BackgroundTransparency = 1
    IntroTitle.Text = "LUAWARE SCRIPT"
    ...
    -- =======================================
    -- ANA PENCERE (Başlangıçta KESİNLİKLE GİZLİ)
    -- =======================================
    local Main = Instance.new("CanvasGroup", sg)
    -- Büyüme efekti için önce küçük başlatıyoruz (480x280)
    Main.Size = UDim2.new(0, 480, 0, 280); Main.Position = UDim2.new(0.5, -240, 0.5, -140)
    Main.BackgroundColor3 = Theme.Main; Main.BorderSizePixel = 0
    Main.GroupTransparency = 1 -- Tamamen saydam
    Main.Visible = false -- Ekrandan tamamen gizli
    Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 6)
    local stroke = Instance.new("UIStroke", Main); stroke.Color = Theme.TopLine; stroke.Thickness = 1

    -- Animasyon Sekansı (Hawk Hub Style)
    task.spawn(function()
        -- 1. Logo ve İsim yavaşça büyüyerek belirir
        TweenService:Create(IntroLogo, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 80, 0, 80), Position = UDim2.new(0.5, -40, 0.5, -50), ImageTransparency = 0
        }):Play()
        task.wait(0.3)
        TweenService:Create(IntroTitle, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
        
        -- Ekranda 1 saniye asılı kalır
        task.wait(1.2)
        
        -- 2. Logo ve İsim kaybolur
        TweenService:Create(IntroLogo, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.5, 0, 0.5, 0), ImageTransparency = 1
        }):Play()
        ...
        -- 3. ANA MENÜ ŞOVU (Pürüzsüzce büyüyerek ekrana gelir)
        Main.Visible = true
        TweenService:Create(Main, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            GroupTransparency = 0,
            Size = UDim2.new(0, 520, 0, 320), -- Orijinal boyutuna ulaşır
            Position = UDim2.new(0.5, -260, 0.5, -160)
        }):Play()
    end)

    -- Arayüz Kodlarının Geri Kalanı...
